...
document.body.addEventListener('keyup', function (event) {
    ...
    const el = event.target;
    const selector = getBestSelector(el);
    // Assertion shortcuts (Shift+key)
    if (event.shiftKey && pause_rec === 'no' && rec_mode === '1') {
        const el = event.target;
        const selector = getBestSelector(el);
        const d_now = Date.now();
        var text = '';
        ...
        if (l_key === 's' && text.length > 0) {
            document.recorded_actions.push(['as_te', selector, text, d_now]);
            saveRecordedActions(); return;
        } else if (l_key === 'e') {
            document.recorded_actions.push(['as_el', selector, '', d_now]);
            saveRecordedActions(); return;
        } else if (l_key === 'p') {
            document.recorded_actions.push(['as_ep', selector, '', d_now]);
            saveRecordedActions(); return;
        } else if (l_key === 'v') {
            document.recorded_actions.push(['as_ev', selector, '', d_now]);
            saveRecordedActions(); return;
        } else if (l_key === 'n') {
            document.recorded_actions.push(['as_en', selector, '', d_now]);
            saveRecordedActions(); return;
        } else if (l_key === 'h') {
            document.recorded_actions.push(['hi_lt', selector, '', d_now]);
            saveRecordedActions(); return;
        } else if (l_key === 'g') {
            document.recorded_actions.push(['savsc', selector, '', d_now]);
            saveRecordedActions(); return;
        }
    }
    skip_input = false;
